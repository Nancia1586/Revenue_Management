...
@endforeach
    <body>
        <a href="#" id="downloadPdf" class="btn btn-success">AS PDF</a>
        <br /><br />
        ...
        <script>
            var xValues = ["Vente", "Achat", "Benefice", "Perte", "Commission"];
            var yValues = [
                {{ $liste->sum('totalvente') }}, {{ $liste->sum('totalachat') }}, {{ $liste->sum('totalbeneficeavecperte') }},{{ $liste->sum('totalperte') }}
               , {{ $liste->sum('beneficecommission') }}
            ];
            var barColors = [
                "#b91d47",
                "#00aba9",
